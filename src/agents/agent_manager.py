...
class AgentManager:
    ...
    async def create_demo_agents(self):
        """Create real YouTube agents using actual credentials"""
        try:
            # Load real YouTube credentials
            import json
            import os
            
            # Try to load real YouTube tokens
            token_file = os.path.join(os.path.dirname(__file__), '..', '..', 'youtube_tokens.json')
            if os.path.exists(token_file):
                ...
                real_agents = [
                    {
                        'id': 'agent_001',
                        'channel_id': 'UC9406DUSpnpRagASMXnhzrQ',  # Real channel ID from logs
                        'channel_name': 'george callan',
                        'access_token': youtube_creds.get('access_token', 'demo_token_1'),
                        'refresh_token': youtube_creds.get('refresh_token', 'demo_refresh_1')
                    }
                ]
                self.logger.info("✅ Using real YouTube credentials for agents")
            else:
                # Fallback to demo if no real tokens
                real_agents = [
                    {
                        'id': 'agent_001',
                        'channel_id': 'UC9406DUSpnpRagASMXnhzrQ',  # Real channel ID
                        'channel_name': 'george callan',
                        'access_token': 'demo_token_1',
                        'refresh_token': 'demo_refresh_1'
                    }
                ]
                self.logger.warning("⚠️ Using demo tokens - uploads may fail")
        except Exception as e:
            ...
            real_agents = [
                {
                    'id': 'agent_001',
                    'channel_id': 'UC9406DUSpnpRagASMXnhzrQ',
                    'channel_name': 'george callan',
                    'access_token': 'demo_token_1',
                    'refresh_token': 'demo_refresh_1'
                }
