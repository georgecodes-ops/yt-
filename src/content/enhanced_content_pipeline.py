    ...
    async def _generate_content_from_idea(self, idea: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            research_data = {
                ...
                'uniqueness_score': idea.get('uniqueness_score', 0.5),
                'content_potential': idea.get('content_potential', 'medium'),
                'news_items': getattr(self.intelligence_aggregator, 'latest_news', [])[:3],
                'trends': getattr(self.intelligence_aggregator, 'current_trends', [])[:3],
                'academic_insights': getattr(self.intelligence_aggregator, 'academic_data', [])[:2]
            }
            
            # Generate content using universal engine with research context
