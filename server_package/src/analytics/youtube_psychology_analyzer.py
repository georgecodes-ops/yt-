...
class YouTubePsychologyAnalyzer:
    ...
    async def _get_audience_alignment(self, content_data: Dict) -> Dict:
        """Check how well content aligns with audience psychology"""
        target_audience = content_data.get('target_audience', 'young_investors')
        audience_profile = self.audience_profiles.get(target_audience, self.audience_profiles['young_investors'])
        
        return {
            'target_audience': target_audience,
            'alignment_score': 0.85,  # Simplified for now
            'primary_triggers': [t.value for t in audience_profile['primary_triggers']],
            'content_recommendations': audience_profile['content_preferences']
        }
