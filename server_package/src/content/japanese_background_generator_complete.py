...
class JapaneseBackgroundGenerator:
    ...
    def generate_background_prompt(self, scene_type: str = "mixed", topic: str = "finance") -> str:
        ...
        # Select infographic elements based on topic (map to available types)
        if "crypto" in topic.lower() or "tech" in topic.lower():
            infographic_type = "ui"  # Use UI elements for tech topics
        ...
        available_elements = self.infographic_elements[infographic_type]
        num_elements = min(2, len(available_elements))
        infographic_elements = random.sample(available_elements, num_elements)
        
        # Create prompt based on scene type
        if scene_type == "traditional":
            elements = self.japanese_elements["traditional"]
            style = "traditional Japanese aesthetic"
            # Select elements
            all_elements = elements["nature"] + elements["architecture"] + elements["cultural"]
            background_elements = random.sample(all_elements, min(3, len(all_elements)))
            color_palette = random.sample(elements["colors"], min(3, len(elements["colors"])))
            ...
            # Select elements
            all_elements = elements["urban"] + elements["technology"] + elements["business"]
            background_elements = random.sample(all_elements, min(3, len(all_elements)))
            color_palette = random.sample(elements["colors"], min(3, len(elements["colors"])))
            texture = random.choice(elements["textures"])
        else:
            ...
            background_elements = [nature_element, architecture_element, cultural_element, urban_element]
            
            # Combine colors and textures from both styles
            combined_colors = self.japanese_elements["traditional"]["colors"] + self.japanese_elements["modern"]["colors"]
            combined_textures = self.japanese_elements["traditional"]["textures"] + self.japanese_elements["modern"]["textures"]
            color_palette = random.sample(combined_colors, min(3, len(combined_colors)))
            texture = random.choice(combined_textures)
    ...
    def generate_character_background_prompt(self, character_context: str = "teaching") -> str:
        ...
        if character_context == "wisdom" or character_context == "traditional":
            ...
            colors = self.japanese_elements["traditional"]["colors"]
            ...
            colors = self.japanese_elements["modern"]["colors"]
        else:
            # Mixed scene
            main_element = random.choice(self.japanese_elements["traditional"]["cultural"])
            secondary_element = random.choice(self.japanese_elements["modern"]["business"])
            colors = self.japanese_elements["traditional"]["colors"] + self.japanese_elements["modern"]["colors"]
        
        color_palette = random.sample(colors, min(2, len(colors)))
