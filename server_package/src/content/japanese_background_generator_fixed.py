...
class JapaneseBackgroundGenerator:
    ...
    def generate_background_prompt(self, scene_type: str = "mixed", topic: str = "finance") -> str:
        ...
        if scene_type in ["traditional", "modern"]:
            # For traditional/modern scenes, we have all the element types
            all_elements = elements["nature"] + elements["architecture"] + elements["cultural"]  # type: ignore
            background_elements = random.sample(
                all_elements, 
                min(3, len(all_elements))
            )
            color_palette = random.sample(elements["colors"], min(3, len(elements["colors"])))  # type: ignore
            texture = random.choice(elements["textures"])  # type: ignore
        else:
            # For mixed scenes, we already selected specific elements
            background_elements = [nature_element, architecture_element, cultural_element, urban_element]  # type: ignore
            color_palette = random.sample(combined_colors, min(3, len(combined_colors)))  # type: ignore
            texture = random.choice(combined_textures)  # type: ignore
